-- ============================================================
-- PABBAS — Phase 5: Production Ordering System
-- Creates orders, order_items tables, sequence, and atomic order creation RPC.
-- ============================================================
...
CREATE TABLE IF NOT EXISTS public.orders (
  ...
  payment_method text NOT NULL DEFAULT 'upi' CHECK (payment_method IN ('upi', 'cash', 'online')),
  ...
  table_reservation_id uuid REFERENCES public.table_reservations(id),
...
-- Link FK from table_reservations to orders if table exists
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'table_reservations_order_id_fkey'
  ) THEN
    ALTER TABLE public.table_reservations
    ADD CONSTRAINT table_reservations_order_id_fkey
...
-- ============================================================
-- ATOMIC RPC: create_customer_order
-- ============================================================
...
BEGIN
  ...
  -- 4. If Dine-in, lock table and validate capacity & availability
  ...
  -- 5. Calculate line totals and subtotal server-side in PAISE
  v_subtotal_paise := 0;
  ...
  -- 6. Delivery Fee Calculation (Server-side)
  ...
  -- 7. Generate Collision-Safe Order Number (e.g. PAB-20260908-1001)
  ...
  -- 10. If Dine-in, create table reservation linked to order
  IF p_order_type = 'dine-in' THEN
